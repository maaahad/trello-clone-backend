...
module.exports = {
  ...
  getWorkspacesByUserId: async (id) => {
    ...
    if (user && user.loggedin) {
      return await Workspace.find(
        { members: { $in: [user._id] } },
        "_id owner title members boards"
      )
        .populate({
          path: "owner",
          select: "_id email",
  ...
  createWorkspace: async ({
    ...
    // first we get all id of the members
    // here all members should have email id
    let membersIds = membersEmails.map(async (email) => {
      const member = await User.findOne({ email }, "_id");
      return member._id;
    });
    const members = membersIds ? [...membersIds, userId] : [userId];
    //  boards is supposed to be an emplty array
